...
const Boltz2 = () => {
  ...
  const [selectedTask, setSelectedTask] = useState('protein_ligand_binding');
  ...
  const handlePredictionResult = async (result: any) => {
    console.log('🎯 handlePredictionResult called with:', result);
    console.log('🎯 Result keys:', Object.keys(result));
    console.log('🎯 Result status:', result.status);
    console.log('🎯 Result affinity:', result.affinity);
    console.log('🎯 Result confidence:', result.confidence);
    
    // Set result immediately - no log enhancement blocking
    setPredictionResult(result);
    setIsLoading(false);
    
    console.log('🎯 Result state set, isLoading set to false');
    console.log('🎯 predictionResult will be:', result);
  ...
  const enhanceResultWithModalLogs = async (result: any): Promise<PredictionResult> => {
    console.log('🔧 enhanceResultWithModalLogs called with:', result);
    try {
      // Try to fetch Modal logs for this job
      const response = await fetch(`/api/v2/jobs/${result.job_id}/logs`);
      console.log('🔧 Logs fetch response status:', response.status);
      if (response.ok) {
        const logData = await response.json();
        console.log('🔧 Logs fetched successfully:', logData);
        return {
          ...result,
    ...
    console.log('🔧 Returning result without logs:', result);
    return result;
  };
