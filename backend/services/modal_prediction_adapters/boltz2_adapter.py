"""
Boltz-2 Modal Prediction Adapter
Handles Boltz-2 specific parameter formatting and result processing
"""
...
class Boltz2Adapter(BaseModalAdapter):
    """Adapter for Boltz-2 biomolecular interaction predictions"""
    
    def prepare_parameters(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Prepare parameters for Boltz-2 Modal function
        ...
        logger.info(f"Prepared Boltz-2 parameters: {len(protein_sequences)} proteins, {len(ligands)} ligands")
        return parameters
    
    def process_result(self, modal_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process Boltz-2 Modal function result
        
        Standardizes the result format and extracts key information
        """
        if not isinstance(modal_result, dict):
            raise ValueError("Boltz-2 result must be a dictionary")
        
        # Extract core result data
        processed = {
            "status": modal_result.get("status", "unknown"),
            "task_type": "protein_ligand_interaction",
            "model_version": modal_result.get("model_version", "boltz2_default"),
        ...
        logger.info(f"Processed Boltz-2 result: status={processed['status']}")
        return processed
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> None:
        """Validate Boltz-2 specific input requirements"""
        super().validate_input(input_data)
        
        # Boltz-2 specific validations
        if "protein_sequences" not in input_data:
            raise ValueError("protein_sequences is required for Boltz-2 predictions")
        ...
        logger.debug("Boltz-2 input validation passed")