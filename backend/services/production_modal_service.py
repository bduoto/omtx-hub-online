...
class ProductionModalService:
    ...
    async def configure_webhooks(self, webhook_base_url: str, webhook_secret: str) -> bool:
        """Configure webhooks for all Modal apps"""
        
        logger.info(f"🔗 Configuring webhooks: {webhook_base_url}")
        ...
        for model_type, config in self.config['models'].items():
            try:
                ...
                logger.info(f"  🔧 Configuring {app_name}.{function_name}")
                
                # Get Modal function
                modal_func = await self.get_modal_function(model_type)
                
                # Configure webhook (conceptual API - actual implementation may vary)
                webhook_config = {
                ...
                if hasattr(modal_func, 'configure_webhook'):
                    modal_func.configure_webhook(webhook_config)
                    success_count += 1
                    logger.info(f"  ✅ Webhook configured for {app_name}.{function_name}")
                else:
                    logger.warning(f"  ⚠️ Webhook configuration not supported for {app_name}")
                
            except Exception as e:
                logger.error(f"  ❌ Failed to configure webhook for {model_type}: {e}")
        
        logger.info(f"🔗 Webhook configuration complete: {success_count}/{total_apps} apps")
        return success_count == total_apps
    
    async def get_execution_status(self, modal_call_id: str) -> Optional[ModalCall]:
        """Get execution status by Modal call ID"""
        return self._active_calls.get(modal_call_id)
    
    async def list_active_executions(self) -> List[ModalCall]:
        """List all active executions"""
        return list(self._active_calls.values())
    
    async def cancel_execution(self, modal_call_id: str) -> bool:
        """Cancel a running execution"""
        if modal_call_id not in self._active_calls:
            return False
        
        try:
            call_info = self._active_calls[modal_call_id]
            
            # Get the Modal function
            modal_func = await self.get_modal_function(call_info.model_type)
            
            # Cancel the execution (conceptual API)
            if hasattr(modal_func, 'cancel'):
                modal_func.cancel(modal_call_id)
            
            # Clean up tracking
            self._active_calls.pop(modal_call_id)
            self._lane_usage[call_info.lane] -= 1
            
            logger.info(f"🛑 Cancelled execution: {modal_call_id}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Failed to cancel execution {modal_call_id}: {e}")
            return False
