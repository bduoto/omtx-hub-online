...
from api.frontend_compatibility import router as frontend_compatibility_router
from api.health_checks import router as health_checks_router
from api.download_endpoints import router as download_router
...
# Add security middleware (includes CORS, security headers, rate limiting)
add_security_middleware(app)

# Include health checks (CRITICAL for Cloud Run)
app.include_router(health_checks_router, tags=["Health Checks"])

# Include frontend compatibility layer (CRITICAL for demo)
app.include_router(frontend_compatibility_router, tags=["Frontend Compatibility"])

# Include download endpoints (CRITICAL for results)
app.include_router(download_router, tags=["Results Download"])

# Include unified endpoints
app.include_router(unified_router, prefix="/api/v4", tags=["Unified API v4"])
